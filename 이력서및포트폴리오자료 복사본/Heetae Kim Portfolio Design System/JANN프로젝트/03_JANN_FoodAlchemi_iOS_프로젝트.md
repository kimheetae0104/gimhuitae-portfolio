...
2. **향미 특성 분석**: 해당 재료의 주요 향기 성분 추출 (예: 딸기 → Furaneol, Gamma-Decalactone)
...
- **알고리즘**: PMI Co-occurrence = `log(P(a,b) / (P(a) × P(b)))` — 자체 코퍼스 공동 출현
...
| 우선순위 | 모델 | 역할 | 특징 |
|---------|------|------|------|
| **1순위** | SSDLite (CoreML) | 오프라인 온디바이스 추론 | 모델 ~5.2MB, iOS 실시간 추론, 무료 |
| **2순위** | GPT-4o Vision | 한국 식재료 + 라벨 텍스트 인식 | 온디바이스 모델 미인식 재료 (김치, 두부 등) 보완 |
| **폴백** | Apple Vision Framework | 두 결과 모두 없을 때 | 최후 안전망 |

- **Parallel 실행**: SSDLite + GPT-4o 동시 호출 → 결과 합산 (중복 제거)
- **이미지 해상도**: 1536px 유지

### 4-2. SSDLite 학습 환경 및 성과
...
- **한영 매핑**: GPT 배치 번역으로 식재료 1,530개 + 향미 특성 1,107개 전부 한국어화 완료
- ✅ 향미 특성명 한국어 조회 구현 완료 (메틸 부티레이트, 1-옥탄올 등 실제 표시)

### 4-4. GPT 레시피 생성

- **모델**: GPT-4o-mini (레시피) / GPT-4o (이미지 인식)
- 향미 페어링 알고리즘 DB 분석 결과(공유 페어링 점수 + 실제 향미 특성명)를 프롬프트에 주입
...
| 분자 설명 | GPT 생성 (AI가 지어냄) | DB 실측 향미 특성명(Furaneol 등) 직접 추출 |
| 한국 식재료 | GPT-4o만 가능, 페어링 DB 없음 | 한국 식재료 전용 SSDLite + 한국 식재료 페어링 DB 구축 |
...
- ✅ 실제 향미 특성명 DB 쿼리 구현 완료 — 분자 설명 DB 실측값 기반
...
- 한국 식재료 SSDLite 모델 — Roboflow로 데이터셋 구축 (김치, 두부, 대파, 된장, 고추장)
...
| **3단계 폴백 구조** | SSDLite → GPT-4o → Apple Vision — 인식 누락 최소화 |
