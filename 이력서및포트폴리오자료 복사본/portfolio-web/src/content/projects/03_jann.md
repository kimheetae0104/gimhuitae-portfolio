...
남은 재료 인식 (입력)  →  향미 페어링 분석 (처리)  →  최적 레시피 생성 (출력)
   SSDLite + GPT-4o        향미 페어링 DB         GPT-4o-mini
```
...
- 향미 페어링 DB 분석 결과(공유 페어링 점수 + 실제 향미 특성명)를 프롬프트에 주입
...
- 🔄 향미 페어링 × 레시피 DB 연결 → 과학적 근거 기반 자동 검색
...
- **Korean 향미 페어링 DB** — 식품안전처/농촌진흥청 오픈 API 활용
- 유통기한 OCR 도입
...
- **학술 데이터셋의 실용화 가능성**: Nature 논문 데이터셋(향미 페어링)이라는 학술 자산을 SQLite DB로 변환해 모바일 앱에 내장한 경험. 학술과 제품의 거리를 좁히는 작업 자체가 차별화 포인트가 됨.
...
**Keywords**: 향미 페어링 분석 · 향미 페어링 · 온디바이스 ML · 제로 웨이스트 푸드테크 · iOS 네이티브 · 하이브리드 비전 파이프라인
